"""
Genomic utilities for ATAC-seq and RNA-seq data processing and model evaluation.

This module provides utilities for:
- ATAC-seq data processing and model evaluation
- RNA-seq data processing and model evaluation  
- Sequence encoding and genomic interval manipulation
- Model input preparation and gradient computation
- Visualization tools for genomic tracks and sequence logos
"""
...
# =============================================================================
# Non-distributed input processing functions
# =============================================================================
...
# =============================================================================
# TFRecord and dataset functions
# =============================================================================
...
# =============================================================================
# Variant and annotation parsing functions
# =============================================================================
...
def parse_gtf_collapsed(gtf_file, chromosome, start, end):
    ...
    return genes
def process_enhancer_annotations(example_gene, directory, input_file='hg38_eg.bed'):
    """Process enhancer annotations for a specific gene using pandas."""
    
    # Read the bed file
    df = pd.read_csv(os.path.join(directory,input_file), sep='\t', header=None, 
                     names=['chrom', 'start', 'end', 'gene', 'significant'])

    # Split on first underscore and extract gene name, then filter for exact match
    df['gene_name'] = df['gene'].str.split('_', n=1).str[0]
    gene_data = df[df['gene_name'] == example_gene].copy()
    
    if gene_data.empty:
        print(f"No data found for gene: {example_gene}")
        return
    
    # Sort by chromosome and start position
    gene_data = gene_data.sort_values(['chrom', 'start'])
    
    # Add row numbers
    gene_data['row_num'] = range(1, len(gene_data) + 1)
    
    # Save all enhancers
    output_file = os.path.join(directory,f"{example_gene}.eg.encoded.bed")
    print(output_file)
    gene_data[['chrom', 'start', 'end', 'row_num']].to_csv(
        output_file, sep='\t', header=False, index=False
    )
    
    # Save significant enhancers only - look for 'TRUE' in the original gene column
    sig_data = gene_data[gene_data['gene'].str.contains('_TRUE_', na=False)].copy()
    if not sig_data.empty:
        sig_output_file = os.path.join(directory,f"{example_gene}.eg.sig.encoded.bed")
        print(sig_output_file)
        sig_data[['chrom', 'start', 'end', 'row_num']].to_csv(
            sig_output_file, sep='\t', header=False, index=False
        )
        
        # Compress and index
        subprocess.run(['bgzip', sig_output_file], check=True)
        subprocess.run(['tabix', f"{sig_output_file}.gz"], check=True)
    
    # Compress and index the main file
    subprocess.run(['bgzip', output_file], check=True)
    subprocess.run(['tabix', f"{output_file}.gz"], check=True)
    
    print(f"Processed {len(gene_data)} enhancers for {example_gene}")
    if not sig_data.empty:
        print(f"Found {len(sig_data)} significant enhancers")
...
# Note: RNA utilities use the same imports as ATAC utilities above
# Import EpiBERT models for different tasks
try:
    from epibert.models import epibert_atac_pretrain as epibert_pretrain
    from epibert.models import epibert_rampage_finetune as epibert_rampage
except ImportError:
    print("Warning: EpiBERT models not found. RNA utilities may not work properly.")
    epibert_pretrain = None
    epibert_rampage = None

# Import enformer if available
try:
    import enformer
except ImportError:
    print("Warning: enformer module not found. Enformer utilities may not work properly.")
    enformer = None
...
# =============================================================================
# EpiBERT model classes for different tasks
# =============================================================================

class epibert_pretrain_model:
    """EpiBERT model for ATAC-seq pretraining tasks."""
    def __init__(self, strategy, model, model_checkpoint):
        if epibert_pretrain is None:
            raise ImportError("epibert_atac_pretrain module not available. Please install EpiBERT package.")
        self.model = model
        
        # Initialize with dummy data for model building
        dummy_seq = tf.data.Dataset.from_tensor_slices([tf.ones((524288,4),dtype=tf.float32)] *6)
        dummy_atac = tf.data.Dataset.from_tensor_slices([tf.ones((131072,1),dtype=tf.float32)]*6)
        dummy_motif = tf.data.Dataset.from_tensor_slices([tf.ones((1,693),dtype=tf.float32)]*6)
        combined_dataset = tf.data.Dataset.zip((dummy_seq, dummy_atac, dummy_motif))
...
class epibert_rampage_model_tpu:
    """EpiBERT model for RAMPAGE finetuning tasks."""
    def __init__(self, strategy, model, model_checkpoint):
        if epibert_rampage is None:
            raise ImportError("epibert_rampage_finetune module not available. Please install EpiBERT package.")
        self.model = model
...
class epibert_rampage_model_gpu:
    """EpiBERT model wrapper for GPU usage without strategy parameter."""
    def __init__(self, model, model_checkpoint):
        ...
        # Call model to create variables
        _ = self.model.predict_on_batch((dummy_seq, dummy_atac, dummy_motif))
        print("Model variables created")
        # Load checkpoint
        ckpt = tf.train.Checkpoint(model=self.model).restore(model_checkpoint).expect_partial().assert_existing_objects_matched()
        print("Model weights loaded")
    
    def contribution_input_grad_dist_simple(self, model_inputs, gradient_mask):
        """Compute gradients without strategy parameter."""
        seq, atac, tf_activity = model_inputs
        gradient_mask = tf.cast(gradient_mask, dtype=tf.float32)
        gradient_mask_mass = tf.reduce_sum(gradient_mask)
        ...
        with tf.GradientTape() as input_grad_tape:
            input_grad_tape.watch(seq)
            input_grad_tape.watch(atac)
            pred_atac, pred_rna, att_matrices = self.model.predict_on_batch(model_inputs)
            
            pred_rna = tf.cast(pred_rna, dtype=tf.float32)
            gradient_mask = tf.cast(gradient_mask, dtype=tf.float32)

            prediction_mask = tf.reduce_sum(gradient_mask * pred_rna) / gradient_mask_mass
        ...
        atac_grads = input_grads_atac[0, :, :]

        return seq, input_grads_seq, atac_grads, pred_rna, att_matrices
...
def return_all_inputs_simple_gpu(interval, atac_dataset, rna_dataset, SEQUENCE_LENGTH,
                      num_bins, resolution, motif_activity,crop_size,output_length,
                      fasta_extractor,mask_indices_list):
    """GPU version of return_all_inputs_simple without strategy parameter."""
    ...
    # For GPU usage, return inputs directly as a tuple instead of distributed dataset
    # example in batch dim 
    ...
    mask = tf.slice(mask, [0,crop_size,0],[-1,output_length-2*crop_size,-1])
    mask_centered = tf.slice(mask_centered, [0,crop_size,0],[-1,output_length-2*crop_size,-1])
    return inputs,masked_atac,target_atac,target_atac_uncropped,rna_arr,masked_atac_reshape, mask, mask_centered
...
class enformer_model:
    def __init__(self):
        if enformer is None:
            raise ImportError("enformer module not available. Please install enformer package.")
