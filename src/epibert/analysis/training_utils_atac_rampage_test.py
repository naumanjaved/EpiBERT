...
def return_train_val_functions(model, optimizers_in,
    ...
    if predict_atac:
        metric_dict['ATAC_PearsonR_test'] = metrics.MetricDict({'PearsonR': metrics.PearsonR(reduce_axis=(0,1))})
        metric_dict['ATAC_R2_test'] = metrics.MetricDict({'R2': metrics.R2(reduce_axis=(0,1))})
        metric_dict['ATAC_PearsonR_test_ho'] = metrics.MetricDict({'PearsonR': metrics.PearsonR(reduce_axis=(0,1))})
        metric_dict['ATAC_R2_test_ho'] = metrics.MetricDict({'R2': metrics.R2(reduce_axis=(0,1))})

    metric_dict['RNA_PearsonR_test'] = metrics.MetricDict({'PearsonR': metrics.PearsonR(reduce_axis=(0,1))})
    metric_dict['RNA_R2_test'] = metrics.MetricDict({'R2': metrics.R2(reduce_axis=(0,1))})
    metric_dict['RNA_PearsonR_test_ho'] = metrics.MetricDict({'PearsonR': metrics.PearsonR(reduce_axis=(0,1))})
    metric_dict['RNA_R2_test_ho'] = metrics.MetricDict({'R2': metrics.R2(reduce_axis=(0,1))})
    ...
    def dist_test_step(inputs): # main train step
        print('tracing training step!') # just to make sure no retracing occurring
        sequence,atac,mask,unmask,target_atac,motif_activity,target_rna,cell_type =inputs
        ...
        metric_dict['RNA_PearsonR_test'].update_state(target_rna, output_rna)
        metric_dict['RNA_R2_test'].update_state(target_rna, output_rna)
        if predict_atac:
            metric_dict['ATAC_PearsonR_test'].update_state(target_atac, output_atac)
            metric_dict['ATAC_R2_test'].update_state(target_atac, output_atac)
        
        return target_rna,output_rna,cell_type

    @tf.function(reduce_retracing=True)
    def dist_test_step_ho(inputs): # main train step
        print('tracing training step!') # just to make sure no retracing occurring
        sequence,atac,mask,unmask,target_atac,motif_activity,target_rna,cell_type =inputs
        ...
        metric_dict['RNA_PearsonR_test_ho'].update_state(target_rna, output_rna)
        metric_dict['RNA_R2_test_ho'].update_state(target_rna, output_rna)
        if predict_atac:
            metric_dict['ATAC_PearsonR_test_ho'].update_state(target_atac, output_atac)
            metric_dict['ATAC_R2_test_ho'].update_state(target_atac, output_atac)
    ...
    def dist_val_step(inputs):  # main validation step
        ...
        return target_rna_agg,output_rna_agg, gene_token, cell_type, target_rna, output_rna
    ...
    return dist_test_step, dist_test_step_ho, dist_val_step,dist_val_step_ho,build_step,metric_dict
...
def return_dataset(gcs_path, tss_bool, split, batch, input_length, output_length_ATAC,
...
def return_distributed_iterators(gcs_path, gcs_path_ho, global_batch_size,
    ...
    test_iterator = return_dataset(gcs_path,False, "test_nonTSS", global_batch_size, input_length,
    ...
    test_iterator_ho = return_dataset(gcs_path_ho, False,"test_nonTSS", global_batch_size, input_length,
                             output_length_ATAC, output_length, crop_size,
                             output_res, max_shift, options, num_parallel_calls,
                             num_epoch, atac_mask_dropout, random_mask_size,
                             log_atac, use_atac, use_seq, seed,
    ...
    test_tss_iterator = return_dataset(gcs_path, True,"test", global_batch_size, input_length,
                                 output_length_ATAC, output_length, crop_size,
    ...
    test_tss_iterator_ho = return_dataset(gcs_path_ho, True, "test", global_batch_size, input_length,
                                 output_length_ATAC, output_length, crop_size,
                                 output_res, max_shift, options_val, num_parallel_calls, num_epoch,
                                 atac_mask_dropout_val, random_mask_size, log_atac,
                                 use_atac, use_seq, seed_val, atac_corrupt_rate,
    ...
    test_tss_iterator_ho_b = return_dataset(gcs_path_ho, True, "test", global_batch_size, input_length,
                                 output_length_ATAC, output_length, crop_size,
                                 output_res, max_shift, options_val, num_parallel_calls, num_epoch,
                                 atac_mask_dropout_val, random_mask_size, log_atac,
                                 use_atac, use_seq, seed_val, atac_corrupt_rate,
    ...
    test_dist=strategy.experimental_distribute_dataset(test_iterator)
    test_dist_it = iter(test_dist)

    test_dist_ho=strategy.experimental_distribute_dataset(test_iterator_ho)
    test_dist_it_ho = iter(test_dist_ho)

    test_tss_dist=strategy.experimental_distribute_dataset(test_tss_iterator)
    test_tss_dist_it = iter(test_tss_dist)

    test_tss_dist_ho=strategy.experimental_distribute_dataset(test_tss_iterator_ho)
    test_tss_dist_it_ho = iter(test_tss_dist_ho)

    test_tss_dist_ho_b=strategy.experimental_distribute_dataset(test_tss_iterator_ho_b)
    test_tss_dist_it_ho_b = iter(test_tss_dist_ho_b)

    return test_dist_it, test_dist_it_ho, test_tss_dist_it,test_tss_dist_it_ho,test_tss_dist_it_ho_b
...
def parse_args(parser):
    ...
    parser.add_argument('--test_examples', type=int,help='test_examples')
    parser.add_argument('--test_examples_ho', type=int,help='test_examples_ho')
    parser.add_argument('--test_TSS', type=int,help='test_TSS')
    parser.add_argument('--test_TSS_ho', type=int,help='test_TSS_ho')
    ...
    parser.add_argument('--predict_atac', type=str, default="False", help= 'predict_atac')
...
def make_plots(y_trues,
    ...
    return np.nanmean(cell_specific_corrs), np.nanmean(gene_specific_corrs),store_df
